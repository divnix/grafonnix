{ POP, nixlib }:
{
  ...
  new = {
  ...
  }: lib.POP.pop
    {
      defaults = {
      evaluator= {
        params= if builtins.isList evaluatorParams then evaluatorParams else [evaluatorParams];
        type= evaluatorType;
      };
      operator= {
        type= operatorType;
      ...
      reducer= {
        params= if builtins.isList reducerParams then reducerParams else [reducerParams];
        type= reducerType;
      };
      type= "query";
    };
